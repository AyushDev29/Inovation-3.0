...
const HackastraRegistration = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Validate photo count (minimum 2, maximum 3)
            if (!files.college_id_photos || files.college_id_photos.length < 2) {
                throw new Error('Please upload at least 2 college ID photos.');
            }
            if (files.college_id_photos.length > 3) {
                throw new Error('Please upload maximum 3 college ID photos.');
            }
            ...
            if (files.college_id_photos && files.college_id_photos.length > 0) {
                try {
                    ...
                    for (let i = 0; i < files.college_id_photos.length; i++) {
                        const file = files.college_id_photos[i];
                        const uploadedPath = await uploadFile(file, `college_id_${i + 1}`);
                    ...
                    {status === 'success' ? (
                        <div className="flex flex-col items-center justify-center py-6 sm:py-8 text-center">
                            ...
                            <div className="relative bg-gradient-to-br from-gray-900/80 via-black/60 to-gray-800/80 border border-white/10 rounded-2xl p-6 mb-6 w-full max-w-md backdrop-blur-sm overflow-hidden">
                                ...
                                <div className="absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr from-emerald-500/10 to-transparent rounded-full blur-xl"></div>
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="text-center mb-5">
                                        <h4 className="text-white font-bold text-lg mb-2 font-orbitron">Join the Dev Team</h4>
                                        <p className="text-gray-300 text-sm leading-relaxed">
                                            Connect with fellow hackers and get real-time updates about the hackathon
                                        </p>
                                    </div>
                                    
                                    <a
                                        href="https://chat.whatsapp.com/L78pZmds525IUjG8biKNlJ"
                                        target="_blank"
                                    ...
                                    >
                                        ...
                                        <span className="relative z-10">Join Hackastra Team</span>
                                    ...
                                    <div className="flex items-center justify-center mt-4 text-xs text-gray-400">
                                        ...
                                        <span>Code collaboration • Real-time updates • Team sync</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-3 md:space-y-4">
                            ...
                            <MultiplePhotoUpload
                                ...
                                required={true}
                                label="Team College ID Photos (2-3 Photos)"
                                uploadMessage="Upload 2-3 College ID Photos"
                                customInstructions={[
                                    "Upload 2-3 separate college ID photos (minimum 2 required, 3rd optional)",
                                    "Ensure all text and photos on IDs are clearly readable",
                                    "Poor quality/blurred photos may lead to disqualification",
                                    "Upload from gallery - no camera capture needed"
                                ]}
                            />
                            ...
                            <div className="space-y-1.5 pt-1">
                                ...
                                {/* Member 3 (Optional) */}
                                <div className="bg-white/5 rounded-md p-2 border border-white/10">
                                    <div className="text-[9px] text-gray-300 mb-1 font-semibold">Member 3 (Optional)</div>
                                    <input
                                        type="text"
                                        name="player3_name"
...
export default HackastraRegistration;